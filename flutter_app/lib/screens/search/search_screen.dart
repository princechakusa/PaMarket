...
const _kCategories = [
  ...
  {'id': 'electronics','name': 'Electronics', 'icon': Icons.computer},
  {'id': 'fashion',    'name': 'Fashion',     'icon': Icons.shopping_bag},
  {'id': 'furniture',  'name': 'Furniture',   'icon': Icons.weekend},
  {'id': 'services',   'name': 'Services',    'icon': Icons.settings},
...
const _kHints = [
  'iPhone', 'Toyota', 'Apartment Harare', 'Software Developer',
  'Land Rover', 'Maize seeds', 'Generator', 'Plumber',
];
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _onSearchChanged(String _) {
    _debounce?.cancel();
    _debounce = Timer(const Duration(milliseconds: 300), () => _doSearch(reset: true));
  }
  ...
  Future<void> _doSearch({bool reset = false}) async {
    ...
    if (reset) {
      setState(() { _loading = true; _searched = true; _hasMore = true; _results = []; });
    }
    try {
  ...
  Future<void> _loadMore() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _results.addAll(_sortResults(data));
        _hasMore    = data.length == _kPageSize;
        _totalCount = _results.length;
        _loadingMore = false;
      });
  ...
  List<Listing> _sortResults(List<Listing> items) {
    ...
    switch (_sortBy) {
      case 'price-low':  list.sort((a, b) => a.price.compareTo(b.price));
      case 'price-high': list.sort((a, b) => b.price.compareTo(a.price));
      default: break;
    }
    if (_condition != 'all') {
      return list.where((l) =>
  ...
  void _resetFilters() {
    ...
    _maxPriceCtrl.clear();
    setState(() {
      _sortBy = 'recent'; _selectedCategory = null; _condition = 'all';
      _selectedCity = null; _priceMin = null; _priceMax = null; _filtersOpen = false;
    });
    _doSearch(reset: true);
  }
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              _SortChip(value: _sortBy, onChanged: (v) {
                setState(() => _sortBy = v);
                if (_results.isNotEmpty) setState(() => _results = _sortResults(_results));
              }),
              const SizedBox(width: 8),
              _FilterToggleButton(
                isOpen: _filtersOpen, badgeCount: _activeFilterCount,
          ...
          Container(
            decoration: BoxDecoration(
              ...
              boxShadow: const [BoxShadow(color: Color(0x29000000), blurRadius: 20, offset: Offset(0, 4))],
            ),
            child: Row(
              children: [
                ...
                Expanded(
                  child: TextField(
                    ...
                    onChanged: _onSearchChanged, onSubmitted: (_) => _doSearch(reset: true),
                    style: const TextStyle(fontFamily: 'Inter', fontSize: 15, color: AppColors.textPrimary),
                    decoration: const InputDecoration(
                      hintText: 'Search all listings…',
                      hintStyle: TextStyle(fontFamily: 'Inter', fontSize: 15, color: AppColors.textMuted),
                ...
                if (_searchCtrl.text.isNotEmpty)
                  GestureDetector(
                    onTap: () { _searchCtrl.clear(); _doSearch(reset: true); },
                    child: const Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _PriceInput(controller: _minPriceCtrl, hint: 'Min', onChanged: (_) {})),
                const Padding(padding: EdgeInsets.symmetric(horizontal: 10),
                    child: Text('to', style: TextStyle(fontFamily: 'Inter', fontSize: 13, color: AppColors.textMuted))),
                Expanded(child: _PriceInput(controller: _maxPriceCtrl, hint: 'Max', onChanged: (_) {})),
              ],
            ),
            const SizedBox(height: 14),
            ...
            Wrap(
              ...
              children: List.generate(_kConditions.length, (i) {
                ...
                return _ChoiceChipItem(
                  label: _kConditionLabels[i], selected: isSelected,
                  onTap: () => setState(() => _condition = _kConditions[i]),
                );
              }),
            ),
            const SizedBox(height: 14),
            ...
            const SizedBox(height: 8),
            _CityDropdown(value: _selectedCity, onChanged: (v) => setState(() => _selectedCity = v)),
                ...
                Expanded(
                  child: OutlinedButton(
                    ...
                    child: const Text('Reset', style: TextStyle(
                        fontFamily: 'Inter', fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                  ),
                ),
  ...
  Widget _buildResultsBar() {
    return Container(
      ...
      child: Row(
        children: [
          Text(
            _loading ? 'Searching…' : '$_totalCount result${_totalCount != 1 ? 's' : ''} found',
          ...
          if (_activeFilterCount > 0)
            GestureDetector(
              ...
              child: const Text('Clear filters', style: TextStyle(
                  fontFamily: 'Inter', fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.primaryBlue)),
            ),
        ],
      ),
    );
  ...
  Widget _buildBody() {
    ...
    if (_loading) return const Center(child: CircularProgressIndicator(color: AppColors.primaryBlue));
  ...
  Widget _buildHints() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Popular searches', style: TextStyle(
              fontFamily: 'Inter', fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textSecondary)),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8, runSpacing: 8,
            children: _kHints.map((h) => GestureDetector(
              onTap: () { _searchCtrl.text = h; _doSearch(reset: true); },
              child: Chip(
                label: Text(h),
                labelStyle: const TextStyle(fontFamily: 'Inter', fontSize: 13, color: AppColors.textPrimary),
                backgroundColor: AppColors.card,
                side: const BorderSide(color: AppColors.border),
                avatar: const Icon(Icons.search, size: 14, color: AppColors.textMuted),
              ),
            )).toList(),
          ),
          const SizedBox(height: 24),
          const Text('Browse Categories', style: TextStyle(
              fontFamily: 'Inter', fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textSecondary)),
          const SizedBox(height: 12),
          GridView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                onTap: () { setState(() => _selectedCategory = c['id'] as String); _doSearch(reset: true); },
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(c['icon'] as IconData, size: 24, color: AppColors.primaryBlue),
                    ),
                    const SizedBox(height: 5),
                    Text(c['name'] as String, style: const TextStyle(
                        fontFamily: 'Inter', fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textPrimary),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.search_off, size: 42, color: AppColors.primaryBlue),
            ),
            const SizedBox(height: 20),
            const Text('No matches', style: TextStyle(
                fontFamily: 'Inter', fontSize: 20, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
            const SizedBox(height: 8),
            const Text('Try a different search term or adjust your filters', style: TextStyle(
                fontFamily: 'Inter', fontSize: 14, color: AppColors.textSecondary), textAlign: TextAlign.center),
            const SizedBox(height: 24),
            OutlinedButton(
              onPressed: _resetFilters,
              style: OutlinedButton.styleFrom(
              ...
              child: const Text('Clear filters', style: TextStyle(
                  fontFamily: 'Inter', fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.primaryBlue)),
            ),
          ],
        ),
      ),
...
class _FilterToggleButton extends StatelessWidget {
  ...
  const _FilterToggleButton({required this.isOpen, required this.badgeCount, required this.onTap});
          ...
          children: [
            ...
            if (badgeCount > 0) ...[
              ...
              Container(
                ...
                child: Center(child: Text('$badgeCount', style: const TextStyle(
                    fontFamily: 'Inter', fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.primaryBlue))),
              ),
            ],
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => showModalBottomSheet<String>(
        ...
        builder: (_) => _SortSheet(currentValue: value, onSelect: onChanged),
      ),
      child: Container(
...
class _ChoiceChipItem extends StatelessWidget {
  ...
  final bool selected;
  final VoidCallback onTap;
  const _ChoiceChipItem({required this.label, this.icon, required this.selected, required this.onTap});
...
class _PriceInput extends StatelessWidget {
  ...
  final String hint;
  final ValueChanged<String> onChanged;
  const _PriceInput({required this.controller, required this.hint, required this.onChanged});

  @override
  Widget build(BuildContext context) {
    return TextField(
      controller: controller, keyboardType: TextInputType.number, onChanged: onChanged,
      style: const TextStyle(fontFamily: 'Inter', fontSize: 13),
      decoration: InputDecoration(
        hintText: hint,
        hintStyle: const TextStyle(fontFamily: 'Inter', fontSize: 13, color: AppColors.textMuted),
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(9), borderSide: const BorderSide(color: AppColors.primaryBlue)),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String?>(
          value: value,
          hint: const Text('All Zimbabwe', style: TextStyle(fontFamily: 'Inter', fontSize: 13, color: AppColors.textMuted)),
          isExpanded: true,
          style: const TextStyle(fontFamily: 'Inter', fontSize: 13, color: AppColors.textPrimary),
          items: [
            const DropdownMenuItem<String?>(value: null,
                child: Text('All Zimbabwe', style: TextStyle(fontFamily: 'Inter', fontSize: 13))),
            ..._kZwCities.map((c) => DropdownMenuItem<String?>(
                value: c, child: Text(c, style: const TextStyle(fontFamily: 'Inter', fontSize: 13)))),
          ],
          onChanged: onChanged,
        ),
      ),
