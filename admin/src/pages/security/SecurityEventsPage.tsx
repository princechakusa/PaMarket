...
export function SecurityEventsPage() {
  ...
  // Real per-severity counts of the currently-loaded page -- replaces a
  // previous "EVENT RATE" sparkline whose bar heights were a deterministic
  // formula (index*17 + rows.length*7) % 75, not an actual time-bucketed
  // query. No such time-series RPC exists, so this shows only what the
  // page genuinely has: a severity breakdown of the loaded rows.
  const severityCounts = SEVERITIES.map((severity) => ({ severity, count: rows.filter((row) => row.severity === severity).length }));
