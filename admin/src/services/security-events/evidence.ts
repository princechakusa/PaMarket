...
export function networkLocationDescription(metadata: unknown, ipSource: string): string {
  if (ipSource === 'restricted') return 'Restricted for this role';
  const value = metadata && typeof metadata === 'object' ? (metadata as Record<string, unknown>).network_location : null;
  if (!value || typeof value !== 'object') return 'Not recorded';
  const geo = value as Record<string, unknown>;
  const area = [geo.city, geo.region, geo.country].filter((part) => typeof part === 'string' && part).join(', ');
  const coordinates = typeof geo.latitude === 'number' && typeof geo.longitude === 'number'
    ? `approximately ${geo.latitude}, ${geo.longitude}` : 'coordinates unavailable';
  return `${area || 'Area unavailable'}; ${coordinates} (Cloudflare IP estimate, not device GPS)`;
}
...
export async function buildSecurityEvidenceHtml(event: SecurityEventDetail, receiptId: string): Promise<string> {
  ...
  const fields: [string, unknown][] = [
    ...
    ['Network location', networkLocationDescription(event.metadata, event.ip_source)],
