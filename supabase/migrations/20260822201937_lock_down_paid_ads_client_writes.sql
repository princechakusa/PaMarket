...
create policy "paid_ads public read active" on public.paid_ads for select to anon, authenticated using (active = true);
create policy "paid_ads admin write" on public.paid_ads for all to authenticated using (public.is_admin()) with check (public.is_admin());;
