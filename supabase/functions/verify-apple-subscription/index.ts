...
// allowNativeAppOrigins. Local dev origins are now only included when the
// ALLOW_DEV_CORS_ORIGINS secret is explicitly set to "true".
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const mapped = isRecruiter
      ? RECRUITER_SUBSCRIPTION_PRODUCTS[productId]
    ...
    let targetId: string;
    if (isRecruiter) {
      const profRes = await db.rpc("get_or_create_recruiter_profile", {
        p_user_id: userId,
      });
      if (profRes.error || !profRes.data) {
        return json(
          {
            error:
              "Could not resolve recruiter profile: " +
              (profRes.error?.message || "unknown error"),
          },
          500
        );
      }
      ...
      if (!businessId)
        return json(
          { error: "businessId is required for a shop subscription purchase" },
          400
        );
    ...
    const existing = await db
      .from(table)
      .select(
        `id,status,subscription_state,expiry_time,user_id,product_id,${targetColumn}`
      )
      .eq("purchase_token", purchaseToken)
      .maybeSingle();
    if (existing.error) {
      console.error(
        "verify-apple-subscription: existing-token lookup failed:",
    ...
    if (
      existing.data &&
      (existing.data as Record<string, unknown>)[targetColumn] !== targetId
    ) {
      return json(
        {
          error:
            "This subscription is already linked to a different PaMarket account target",
        },
        409
      );
    }
    ...
    if (existing.data) {
      ...
      if (insertRes.error || !insertRes.data) {
        if (
          String(insertRes.error?.message || "").includes("unique") ||
          String(insertRes.error?.message || "").includes(
            "uq_play_subscription_token"
          ) ||
          String(insertRes.error?.message || "").includes(
            "uq_play_recruiter_subscription_token"
          )
        ) {
          return json(
            {
              error:
                "This transaction is already being processed. Please use Restore Purchases.",
            },
            409
          );
        }
        return json(
          {
            error: "Could not record subscription: " + insertRes.error?.message,
          },
          500
        );
      }
    ...
    if (!sub.ok) {
      const failRes = await db
        .from(table)
        .update({ status: "failed", verification_error: sub.reason })
        .eq("id", rowId);
      if (failRes.error)
        console.error(
          "verify-apple-subscription: failed to record verification-failure status:",
          failRes.error.message
        );
      return json(
        { error: "Subscription verification failed: " + sub.reason },
        402
      );
    }
    if (
      sub.originalTransactionId &&
      sub.originalTransactionId !== purchaseToken
    ) {
      return json({ error: "Original transaction mismatch" }, 402);
    }
    if (sub.appAccountToken && sub.appAccountToken !== userId) {
      return json(
        { error: "This purchase belongs to a different PaMarket account" },
        403
      );
    }
    if (sub.productId && sub.productId !== productId) {
      const failRes = await db
        ...
        .update({
          status: "failed",
          verification_error: `Product mismatch: expected ${productId}, got ${sub.productId}`,
        })
        .eq("id", rowId);
      if (failRes.error)
        console.error(
          "verify-apple-subscription: failed to record product-mismatch status:",
          failRes.error.message
        );
    ...
    if (verifiedRes.error) {
      console.error(
        "verify-apple-subscription: failed to record verified status:",
        verifiedRes.error.message
      );
      return json(
        {
          error:
            "Subscription verified with Apple but could not be recorded: " +
            verifiedRes.error.message,
        },
        500
      );
    ...
    if (activateRes.error || activateRes.data?.ok !== true) {
      console.error(
        "[BILLING_ALERT] verify-apple-subscription: verified-but-activation-failed:",
        activateRes.error?.message || activateRes.data?.msg
      );
      return json(
        {
          error:
            "Subscription verified but activation failed: " +
    ...
    if (activateRes.data?.granted !== true) {
      return json(
        {
          error: "Apple reports that this subscription is not currently active",
        },
        402
      );
    }
