...
import { verifyNotificationPayload, verifyTransactionJWS, fetchAppleSubscriptionStatus } from '../_shared/apple-iap.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('apple-notifications-webhook: rejected unverifiable payload:', (e as Error).message);
    ...
    try {
      ...
      console.error('apple-notifications-webhook: could not verify embedded transaction:', (e as Error).message);
    ...
    const isConsumable = decoded.type === 'Consumable' || decoded.type === 'Non-Consumable';
    ...
    if (isConsumable && SUBSCRIPTION_REFUND_TYPES.has(notificationType)) {
      const revokeRes = await db.rpc('revoke_play_purchase', { p_purchase_token: decoded.transactionId });
      if (revokeRes.error) {
        console.error('[BILLING_ALERT] apple-notifications-webhook: revoke_play_purchase failed:', revokeRes.error.message);
        return json({ error: revokeRes.error.message }, 500); // retry
      }
      console.log('apple-notifications-webhook: refunded consumable processed:', JSON.stringify(revokeRes.data));
    ...
    if (!originalTransactionId) return json({ ok: true, skipped: 'no originalTransactionId' });
    ...
    const existing = await db.from('play_subscriptions').select('id').eq('purchase_token', originalTransactionId).maybeSingle();
    ...
    if (existing.data) {
      ...
      const existingRecruiter = await db.from('play_recruiter_subscriptions').select('id').eq('purchase_token', originalTransactionId).maybeSingle();
    ...
    if (verifiedRes.error) {
      console.error('apple-notifications-webhook: failed to record verified status for', rowId, ':', verifiedRes.error.message);
    ...
    if (activateRes.error || activateRes.data?.ok !== true) {
      console.error('[BILLING_ALERT] apple-notifications-webhook: activation failed for', rowId, ':', activateRes.error?.message || activateRes.data?.msg);
    ...
    if (sub.subscriptionState !== 'active' && sub.subscriptionState !== 'in_grace_period') {
      ...
      if (rowRes.error) {
        console.error('apple-notifications-webhook: could not read', downgradeIdColumn, 'for downgrade, rowId', rowId, ':', rowRes.error.message);
      ...
      if (targetId) {
        ...
        if (expireRes.error) {
          console.error('apple-notifications-webhook: failed to expire', lifecycleTable, 'for', targetId, ':', expireRes.error.message);
        ...
        if (downgradeRes.error) {
          console.error('[BILLING_ALERT] apple-notifications-webhook: FAILED to downgrade', downgradeTable, targetId, 'to free plan:', downgradeRes.error.message);
    ...
    console.error('apple-notifications-webhook error:', message, isParseFailure ? '(treated as non-retryable parse failure)' : '(treated as transient — requesting retry)');
