...
// CONSUMABLE purchases: listing boosts (boost_1day/7day/30day, plus
// job_boost_7day/30day which shares the same mechanism), business
// featured-slot packs (featured_slot_pack_1/3), job posting credit packs
// (job_credit_pack_1/5), and rental listing featured placement
// (rental_featured_slot_7day/30day). Activates the corresponding
// entitlement only after Google confirms the purchase is real and
// unconsumed. This is the ONLY code path that can boost a listing, grant
// extra featured slots, grant job credits, or feature a rental listing via
// Play Billing — the client never grants its own entitlement.
// (Subscriptions are handled by the separate verify-play-subscription
// function, since Play models them very differently — recurring state, not
// a one-shot consumable.)
//
// Call shape (boost):          POST { listingId: uuid, productId: 'boost_1day'|'boost_7day'|'boost_30day'|'job_boost_7day'|'job_boost_30day', purchaseToken, orderId? }
// Call shape (slot pack):      POST { businessId: uuid, productId: 'featured_slot_pack_1'|'featured_slot_pack_3', purchaseToken, orderId? }
// Call shape (job credit):     POST { productId: 'job_credit_pack_1'|'job_credit_pack_5', purchaseToken, orderId? }
// Call shape (rental featured): POST { listingId: uuid, productId: 'rental_featured_slot_7day'|'rental_featured_slot_30day', purchaseToken, orderId? }
// Auth: caller must be a signed-in user (their own JWT, not service-role).
//
// Flow (identical for all product families, just a different target table/RPC):
...
const ALLOWED_ORIGINS = new Set([
  ...
  // directly from the app (billing.js) to verify Play purchases, so without
...
// is the production-safety check: any family still marked 'planned' in the
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // AWAITED with one retry — consumption doubles as the acknowledgement
    // Google requires within 3 days; a silently dropped consume would get
