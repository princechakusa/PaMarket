...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return json({ error: 'Purchase verification failed: ' + (e as Error).message }, 402);
    }
    if (decoded.productId !== productId) {
      return json({ error: `Product mismatch: expected ${productId}, got ${decoded.productId}` }, 402);
    }
    ...
    const existing = await db.from(table).select('id, status, expiry_time, user_id').eq('purchase_token', ledgerToken).maybeSingle();
    if (existing.error) {
      console.error('verify-apple-purchase: existing-token lookup failed:', existing.error.message);
    ...
    if (existing.data) {
      ...
      if (existing.data.status === 'verified') {
        ...
        if (activateRes.error || !activateRes.data?.ok) {
          console.error('verify-apple-purchase: activation failed (retry branch):', activateRes.error?.message || activateRes.data?.msg);
    ...
    const verifiedRes = await db.from(table).update({
      ...
      purchase_time: decoded.purchaseDate ? new Date(decoded.purchaseDate).toISOString() : null,
      verified_at: new Date().toISOString(),
    }).eq('id', purchaseRowId);
    if (verifiedRes.error) {
      console.error('verify-apple-purchase: failed to record verified status:', verifiedRes.error.message);
      return json({ error: 'Purchase verified with Apple but could not be recorded: ' + verifiedRes.error.message }, 500);
    }

    const activateRes = await db.rpc(activateFn, { [activateParam]: purchaseRowId });
    if (activateRes.error || !activateRes.data?.ok) {
      console.error('[BILLING_ALERT] verify-apple-purchase: verified-but-activation-failed:', activateRes.error?.message || activateRes.data?.msg);
