...
FORMATTING RULES (apply to every placement): Never use an em dash (—) or en dash (–) anywhere, use a comma, period, or parentheses instead. Never use "***", "**", markdown syntax, or asterisks for emphasis of any kind. Write clean plain text only, no markdown headers (#), no bullet asterisks. For LinkedIn specifically, if you want a heading or emphasized word to visually stand out, convert those specific words to Unicode bold characters (the 𝗠𝗮𝘁𝗵𝗲𝗺𝗮𝘁𝗶𝗰𝗮𝗹 𝗔𝗹𝗽𝗵𝗮𝗻𝘂𝗺𝗲𝗿𝗶𝗰 𝗕𝗼𝗹𝗱 Unicode block, e.g. 𝗕𝗶𝗴 𝗡𝗲𝘄𝘀), since LinkedIn does not render markdown and this is the only way real bold text appears on a LinkedIn post. Use this sparingly, for a short heading-like opening line and 1-2 key phrases at most, not the whole post.
