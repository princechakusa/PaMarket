...
(function () {
  ...
  function listingCard(l) {
    ...
    return '' +
      '<a href="detail.html?id=' + encodeURIComponent(l.id) + '" class="group rounded-xl border border-outline-variant bg-surface-container-lowest overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">' +
  ...
  function fetchPage() {
    var qp = baseQP().slice();
    qp.push('select=id,title,price,currency,category,province,city,photos,created_at,condition,business_id,seller_name');
  ...
  function refreshResults() {
    ...
    Promise.all([fetchTotal(), fetchPage()]).then(function (r) {
      ...
      grid.innerHTML = rows.length ? rows.map(listingCard).join('') :
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('viewGridBtn').addEventListener('click', function () {
      document.getElementById('resultsGrid').className = 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-space-base';
      this.className = 'w-9 h-9 flex items-center justify-center bg-primary text-on-primary'; this.setAttribute('aria-pressed', 'true');
      var lb = document.getElementById('viewListBtn'); lb.className = 'w-9 h-9 flex items-center justify-center text-on-surface-variant'; lb.setAttribute('aria-pressed', 'false');
    });
    document.getElementById('viewListBtn').addEventListener('click', function () {
      document.getElementById('resultsGrid').className = 'grid grid-cols-1 gap-space-base';
      this.className = 'w-9 h-9 flex items-center justify-center bg-primary text-on-primary'; this.setAttribute('aria-pressed', 'true');
      var gb = document.getElementById('viewGridBtn'); gb.className = 'w-9 h-9 flex items-center justify-center text-on-surface-variant'; gb.setAttribute('aria-pressed', 'false');
    });
