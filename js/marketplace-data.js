...
(function (global) {
  ...
  // ── Admin: paid ads management (paid_ads, is_admin() write RLS) ───
  // All of these send the user's access_token; the "paid_ads admin write"
  // policy (is_admin()) is the real gate — a non-admin's writes are rejected
  // server-side regardless of what the client does.
  function adminFetchAds() {
    var s = getSession();
    if (!s || !s.access_token) return Promise.resolve([]);
    return fetch(SB_URL + '/rest/v1/paid_ads?select=*&order=created_at.desc&limit=200', {
      headers: { apikey: SB_KEY, Authorization: 'Bearer ' + s.access_token },
    }).then(function (res) { return res.ok ? res.json() : []; }).catch(function () { return []; });
  }

  // Create an ad. Mirrors the app's column map so an ad made on the website
  // renders identically in the app (and vice versa). `title` is set to the
  // headline too, because the website homepage renders paid_ads by `title`.
  // Falls back to the minimal column set if the live table predates the rich
  // columns, so it never hard-fails.
  function adminSaveAd(ad) {
    var s = getSession();
    if (!s || !s.access_token) return Promise.reject(new Error('not-authenticated'));
    var id = (window.crypto && crypto.randomUUID) ? crypto.randomUUID() : ('ad_' + Date.now() + Math.random().toString(36).slice(2, 8));
    var full = {
      id: id,
      title: ad.headline || ad.title || 'Sponsored',
      business_name: ad.businessName || null,
      headline: ad.headline || null,
      tagline: ad.tagline || null,
      image_url: ad.imageUrl || null,
      bg_color: ad.bgColor || '#1A3A8F',
      type: ad.type || 'banner',
      placement: ad.placement || 'home',
      target_cat: ad.targetCat || null,
      listing_id: ad.listingId || null,
      link_url: ad.linkUrl || null,
      starts_at: ad.startsAt || null,
      ends_at: ad.endsAt || null,
      price_paid: Number(ad.pricePaid) || 0,
      active: ad.active !== false,
      impressions: 0, clicks: 0,
      created_at: new Date().toISOString(),
    };
    function post(body) {
      return fetch(SB_URL + '/rest/v1/paid_ads', {
        method: 'POST',
        headers: { apikey: SB_KEY, Authorization: 'Bearer ' + s.access_token, 'Content-Type': 'application/json', Prefer: 'return=representation' },
        body: JSON.stringify(body),
      }).then(function (res) {
        if (res.ok) return res.json().then(function (rows) { return { ok: true, ad: rows[0] || null }; });
        return res.text().then(function (t) {
          // Rich column missing on an older table → retry with the minimal
          // schema the website homepage actually needs.
          if (body.business_name !== undefined && /column|schema cache|PGRST204/i.test(t)) {
            return post({ id: body.id, title: body.title, image_url: body.image_url, link_url: body.link_url,
              placement: body.placement, active: body.active, starts_at: body.starts_at, ends_at: body.ends_at,
              impressions: 0, clicks: 0 });
          }
          if (/permission|denied|row-level|42501/i.test(t)) throw new Error('Not authorized — admin only.');
  ...
  function adminSetAdActive(adId, active) {
    var s = getSession();
    if (!s || !s.access_token) return Promise.reject(new Error('not-authenticated'));
    return fetch(SB_URL + '/rest/v1/paid_ads?id=eq.' + esc(adId), {
      method: 'PATCH',
      headers: { apikey: SB_KEY, Authorization: 'Bearer ' + s.access_token, 'Content-Type': 'application/json', Prefer: 'return=representation' },
  ...
  function adminDeleteAd(adId) {
    ...
    return fetch(SB_URL + '/rest/v1/paid_ads?id=eq.' + esc(adId), {
      ...
      return res.text().then(function (t) { throw new Error(t || ('delete failed: ' + res.status)); });
    });
  }

  // Upload an ad image to R2 (reuses the listing-photo edge function; the key
  // prefix differs only cosmetically).
  function uploadAdImage(blob, contentType) { return uploadListingPhoto(blob, contentType); }
  ...
  global.PM.adminFetchAds = adminFetchAds;
  global.PM.adminSaveAd = adminSaveAd;
  global.PM.adminSetAdActive = adminSetAdActive;
  global.PM.adminDeleteAd = adminDeleteAd;
  global.PM.uploadAdImage = uploadAdImage;
})(window);
