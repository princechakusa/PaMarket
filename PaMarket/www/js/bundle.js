/* PaMarket bundle — built 2026-05-28T14:26:09Z */
    ...
    try {
      ...
      try {
        ...
        const [sentRes, recvRes] = await Promise.all([
          sb.from('messages').select('conversation_id,sender_id,sender_name').eq('sender_id', u.id).order('created_at',{ascending:false}).limit(50),
...
(function(H) {
  ...
  H.loadProfile = async function(userId) {
    ...
    var res = await c.from('profiles')..select('id,created_at').eq('id',userId).single();
  ...
  H.syncNotifications = async function () {
    ...
    try {
      const res = await c.from('notifications')
        ..select('id,created_at').eq('user_id', u.id)
    ...
    async syncAllMessages() {
      ...
      try {
        ...
        const [msgsRes, convosRes] = await Promise.all([
          sb.from('messages')..select('id,created_at').order('created_at', { ascending: false }).limit(150),
          sb.from('conversations')..select('id,created_at').limit(50)
        ]);
...
(function () {
  ...
  async function handleOAuthSession(session) {
    ...
    try {
      var pr = await window.supabase.from('profiles')..select('id,created_at').eq('id', userId).single();
      var profile = pr.data;
      if (!profile) {
