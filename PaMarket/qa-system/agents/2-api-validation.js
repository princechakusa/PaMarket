    ...
    if (!error && (count !== null && !isNaN(count))) {
      ...
      if (count > 200) {
        ...
        issues.push({
          ...
          rootCause: 'app.js fetchListingsFromSupabase() uses .limit(50) with no pagination. Listings beyond position 200 (ordered by created_at DESC) are never fetched and never appear in any feed or search.',
          fix: {
            type: 'javascript',
            file: 'www/js/app.js',
            fn: 'fetchListingsFromSupabase',
            description: 'Add cursor-based pagination: fetch in pages and append to H.state.listings',
            code: `// In fetchListingsFromSupabase(), replace the single .limit(50) query
