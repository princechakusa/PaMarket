...
(function (H) {
  ...
  // Cache for the async purchase-history fetch — MyActivity's own render is
  // synchronous, so the first render shows a loading state and _after()
  // triggers the real fetch + a re-render once it resolves (same pattern as
  // the recently-viewed-listing backfill below).
  let _purchaseHistoryCache = null;

  const _PURCHASE_LABELS = {
    boost: 'Listing Boost', slotPack: 'Featured Slot Pack', jobCredit: 'Job Posting Credits',
    shopSub: 'Shop Subscription', recruiterSub: 'Recruiter Subscription', rentalFeatured: 'Rental Featured Slot'
  };
  const _STATUS_COLORS = {
    consumed: '#16A34A', verified: '#16A34A', active: '#16A34A',
    pending: '#B45309', failed: '#DC2626', expired: '#71717A', canceled: '#71717A'
  };

  function _purchaseRow(p) {
    const label = _PURCHASE_LABELS[p.kind] || p.productId;
    const displayStatus = p.subscriptionState || p.status;
    const color = _STATUS_COLORS[displayStatus] || 'var(--sub)';
    const detail = p.extraSlots ? `+${p.extraSlots} slot${p.extraSlots === 1 ? '' : 's'}`
      : p.credits ? `${p.credits} credit${p.credits === 1 ? '' : 's'}`
      : p.durationDays ? `${p.durationDays} days`
      : p.productId;
    return `<div style="display:flex;align-items:center;gap:12px;padding:13px 16px;border-top:1px solid var(--border)">
      <div style="flex:1;min-width:0">
        <div style="font-size:13.5px;font-weight:700;color:var(--text)">${H.escHtml(label)}</div>
        <div style="font-size:11.5px;color:var(--sub);margin-top:2px">${H.escHtml(detail)} &middot; ${new Date(p.t).toLocaleDateString()}</div>
      </div>
      <div style="font-size:11px;font-weight:700;color:${color};text-transform:capitalize">${H.escHtml(displayStatus)}</div>
    </div>`;
  }
  ...
  pages.MyActivity = function () {
    ...
    const applicationsCard = `${sectionLabel('Job Applications')}
      <div onclick="H.openInner('AppliedJobs')" style="margin:0 16px 8px;background:var(--card);border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer">
        <div style="width:38px;height:38px;border-radius:10px;background:var(--blue-light,#EEF2FF);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:var(--blue)">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="7" width="20" height="13" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg>
        </div>
        <div style="flex:1"><div style="font-size:13.5px;font-weight:700;color:var(--text)">My Applications</div><div style="font-size:11.5px;color:var(--sub)">${appCount} job application${appCount === 1 ? '' : 's'}</div></div>
        <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="var(--sub)" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
      </div>`;

    const purchaseSection = _purchaseHistoryCache === null
      ? `${sectionLabel('Purchase History')}<div style="margin:0 16px 8px;background:var(--card);border-radius:14px;padding:24px 16px;text-align:center;font-size:13px;color:var(--sub)">Loading…</div>`
      : _purchaseHistoryCache.length
        ? `${sectionLabel('Purchase History')}
           <div style="margin:0 16px 8px;background:var(--card);border-radius:14px;overflow:hidden">
             ${_purchaseHistoryCache.map(_purchaseRow).join('')}
           </div>`
        : `${sectionLabel('Purchase History')}${emptyCard('No purchases yet')}`;
  ...
  // Viewed" shows the full history instead of silently dropping them. Also
  // triggers the async purchase-history fetch (see _purchaseHistoryCache).
  pages.MyActivity_after = function () {
    ...
    if (_purchaseHistoryCache === null && typeof H.fetchPurchaseHistory === 'function') {
      H.fetchPurchaseHistory().then(list => {
        _purchaseHistoryCache = list;
        if (H.currentPageName === 'MyActivity') H.renderPage('MyActivity', H.currentPageParams);
      });
    }
  };
