...
(function (H) {
  ...
  pages.MyListings_after = function () {
    ...
    H._myListings = {
      ...
      del: (id) => { if(!window.confirm('Delete this listing permanently?'))return; if(H._deletedListingIds)H._deletedListingIds[id]=Date.now(); H.state.listings=(H.state.listings||[]).filter(x=>x.id!==id); H.saveState(); H.toast('Listing deleted'); H.renderPage('MyListings'); if(window.supabase&&typeof window.supabase.from==='function')window.supabase.from('listings').delete().eq('id',id).then(null,function(){}); },
