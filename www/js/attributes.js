...
(function (H) {
  ...
  // PaMarket's own subcategory taxonomy (Zim-flavoured). Used both when posting
  // (what are you listing?) and when browsing (tap a category -> pick a type).
  H.SUBCATEGORIES = {
    property: [
      { key:'houses',     label:'Houses' },
      { key:'flats',      label:'Flats & Apartments' },
      { key:'cottages',   label:'Cottages & Garden Flats' },
      { key:'townhouses', label:'Townhouses & Clusters' },
      { key:'stands',     label:'Stands & Land' },
      { key:'commercial', label:'Commercial & Office' },
      { key:'student',    label:'Student Accommodation' }
    ],
    vehicles: [
      { key:'cars',       label:'Cars' },
      { key:'bakkies',    label:'Bakkies & Trucks' },
      { key:'suvs',       label:'SUVs & 4x4' },
      { key:'kombis',     label:'Kombis & Buses' },
      { key:'motorbikes', label:'Motorbikes' },
      { key:'trailers',   label:'Trailers' },
      { key:'parts',      label:'Spares & Parts' },
      { key:'tyres',      label:'Tyres & Rims' }
    ],
    rooms: [
      { key:'single',         label:'Single Rooms' },
      { key:'shared',         label:'Shared Rooms' },
      { key:'cottage',        label:'Cottages' },
      { key:'full-house',     label:'Full House Share' },
      { key:'self-contained', label:'Self-Contained' },
      { key:'student',        label:'Student Digs' }
    ],
    electronics: [
      { key:'phones',      label:'Phones & Tablets' },
      { key:'computers',   label:'Laptops & Computers' },
      { key:'tvs',         label:'TVs & Monitors' },
    ...
    furniture: [
      { key:'sofas',     label:'Sofas & Lounge' },
      { key:'beds',      label:'Beds & Bedroom' },
      { key:'dining',    label:'Dining & Kitchen' },
      { key:'office',    label:'Office Furniture' },
      { key:'outdoor',   label:'Garden & Outdoor' },
      { key:'wardrobes', label:'Wardrobes & Storage' },
      { key:'curtains',  label:'Curtains & Blinds' },
      { key:'rugs',      label:'Rugs & Carpets' },
      { key:'lighting',  label:'Lighting & Fans' },
    ...
    services: [
      ...
      { key:'transport',    label:'Transport & Moving' },
      { key:'professional', label:'Professional Services' }
    ],
    agriculture: [
      { key:'livestock', label:'Livestock' },
      { key:'poultry',   label:'Poultry' },
      { key:'produce',   label:'Crops & Produce' },
      { key:'seeds',     label:'Seeds & Seedlings' },
      { key:'equipment', label:'Farm Equipment' },
      { key:'irrigation',label:'Irrigation' },
      { key:'feed',      label:'Feed & Fertiliser' }
    ],
    pets: [
      { key:'dogs',        label:'Dogs' },
      { key:'cats',        label:'Cats' },
    ...
    kids: [
      ...
      { key:'cots',     label:'Cots & Beds' },
      { key:'carseats', label:'Car Seats' },
      { key:'school',   label:'School Items' }
    ]
  };

  H.subcatLabel = function (cat, key) {
    var list = H.SUBCATEGORIES[cat] || [];
    var f = list.filter(function (s) { return s.key === key; })[0];
    return f ? f.label : '';
  };
