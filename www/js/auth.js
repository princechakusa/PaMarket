...
(function(H) {
  ...
  async function _oauthInCap(c, provider) {
    ...
    if (Browser && App) {
      ...
      const onUrl = async function (event) {
        ...
        // Close the in-app browser FIRST — before the (slow) code exchange — so
        // the Custom Tab / SFSafariViewController is dismissed immediately and
        // the still-visible Google page can't linger or flash over the app while
        // the async exchange runs. Fire-and-forget (no await): awaiting close()
        // was itself part of what left the Google page on screen for a frame.
        try { if (Browser.close) Browser.close(); } catch (e) {}
        try {
  ...
  H._termsText = function() {
    return '<div class="doc-content">'
      ...
      + '<h3>10A. Paid Features and Google Play Billing</h3>'
      + '<p>Separately from the peer-to-peer marketplace transactions described in Section 10 above, PaMarket offers certain optional paid platform features, including listing boosts, featured slot packs, Business Shop subscriptions (Starter, Pro, Premium), Recruiter subscriptions, job posting credit packs, job boosts, and featured rental slots. Unlike buyer-seller transactions, these paid features ARE sold by PaMarket directly to you, and are processed exclusively through Google Play Billing.</p>'
      + '<ul>'
      + '<li><strong>Payment processing:</strong> all paid features are purchased and billed through Google Play Billing using the payment method registered to your Google account. PaMarket does not receive, process, or store your card or payment details at any time.</li>'
      + '<li><strong>Subscriptions:</strong> Business Shop and Recruiter subscriptions renew automatically each billing period until cancelled. You can view, manage, or cancel any subscription at any time via Google Play Store - Payments and subscriptions - Subscriptions. PaMarket cannot cancel a Google Play subscription on your behalf.</li>'
      + '<li><strong>One-time purchases:</strong> boosts, featured slot packs, job posting credits, job boosts, and featured rental slots are one-time consumable purchases that grant the described benefit for the stated duration or quantity and do not renew automatically.</li>'
      + '<li><strong>Refunds:</strong> refund requests for any paid feature are handled through Google Play\'s refund process (Google Play Store - Order history - Report a problem) and are subject to Google Play\'s refund policy. If a purchase completes but the corresponding feature does not activate in the app, contact us at info@pamarketzw.com with your Google Play order number and we will investigate and correct it.</li>'
      + '<li><strong>No effect on free features:</strong> posting listings, applying for jobs, messaging, and browsing the marketplace remain entirely free. Paid features only provide additional visibility or capacity on top of the free platform.</li>'
      + '</ul>'
  ...
  H._privacyText = function() {
    return '<div class="doc-content">'
      ...
      + '<p><strong>Purchase data:</strong> if you buy a paid feature (listing boost, featured slot pack, Business Shop or Recruiter subscription, job posting credits, job boosts, or featured rental slots), we receive a purchase token, product ID, purchase status, and subscription renewal status from Google Play Billing so we can activate and manage the feature on your account. We never receive or store your card number, bank details, or Google account password.</p>'
